...
const verifyLoginShellScript = await read('.devcontainer/scripts/verify-login-shell.sh');
...
describe('DevContainer contract', () => {
	...
	test('verifies Bun, Playwright, Chromium, WebKit and the shell toolchain', () => {
		...
		expect(verifyEnvScript).toContain('bash .devcontainer/scripts/verify-login-shell.sh');
		expect(verifyEnvScript).not.toContain('!= "/usr/bin/zsh"');
		expect(verifyLoginShellScript).toContain('readlink -f --');
		expect(verifyLoginShellScript).toContain('observed login shell');
		expect(verifyLoginShellScript).toContain('expected Zsh binary');
